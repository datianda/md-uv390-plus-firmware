...
try:
    from dmr_enc_sms import build_encrypted_sms, build_clear_sms
except ImportError:
    build_encrypted_sms=None; build_clear_sms=None
try:
    import serial
...
def main():
    ...
    ap.add_argument("--clear-sms", dest="clear_sms", help="build+send a CLEARTEXT DMR SMS (IP/UDP/TMS, no ENC header) — mirrors the firmware clear path")
    ...
    elif getattr(a, "clear_sms", None) is not None:
        if build_clear_sms is None: sys.exit("dmr_enc_sms.py must be next to this tool")
        bursts=build_clear_sms(a.clear_sms, a.dst, a.src, group=a.group, seq=a.seq or 0x90, ipid=a.ipid)
        print("CLEARTEXT SMS %r -> %d bursts (no ENC header)" % (a.clear_sms, len(bursts)))
